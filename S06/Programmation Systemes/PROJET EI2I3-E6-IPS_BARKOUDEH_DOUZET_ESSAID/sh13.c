...
char gbuffer[256];            // Buffer des messages recus du server sur le thread 
char gServerIpAddress[256];   // Variable stockant l'adresse IP du server  
int gServerPort;              // Varaible stockant le port de communication avec le server 
char gClientIpAddress[256];   // Stocke l'adresse IP du Thread reseau du client 
int gClientPort;              // Stocke le port du thread reseau de communication avec le client 
char gName[256];              // Varaible qui stocke le nom du client 
char gNames[4][256];          // Tableau des client acceptés par le server 
int gId;                      // Indice du client dans le tableau des joueurs 
int joueurSel;                // Indice du joueur selectionné 
int objetSel;                 // Indice de l'objet selectionné 
int guiltSel;                 // Indice du coupable 
int guiltGuess[13];
int tableCartes[4][8];        // Tabeleau des cartes de tous les joueurs :On le met à jour avec les demandes d'objets
int b[3];                  
int goEnabled;                // Flag permet d'activer le bouton Go 
int connectEnabled;           // Flag permettant d'afficher le bouton de connexion 
int joueur_win;               // Le numéro du joueur qui a gagné le jeu 
int joueur_echec;             // Variable "flag" == 1,si un joueur a échoué à deviner le coupable 
...
void *fn_serveur_tcp(void *arg)
{
       ...
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1)
        {
            ...
            //printf("%s",gbuffer);
            synchro=1;
            while (synchro);  // Boucle permet de synchroniser entre tous les messages des joueurs 
     }
}

// Fonction permettant d'envoyer des messages au server par le thread réseau créé 
// La fonction prend en paramètre l'adresse du server, le port de communication et le message à envoyer 
void sendMessageToServer(char *ipAddress, int portno, char *mess)
{
    ...
    char sendbuffer[256]; // Buffer sotckant le message à envoyer 
    ...
    bcopy((char *)server->h_addr,
         ...
         server->h_length);
    serv_addr.sin_port = htons(portno);
    if (connect(sockfd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) < 0) // Echec de connexion avec le server 
        {
        ...
        n = write(sockfd,sendbuffer,strlen(sendbuffer)); // Fonciton write permettant d'envoyer le message à travers la connexion qui a été établie. 

    close(sockfd); // On ferme les sockets créés 
}
...
int main(int argc, char ** argv)
{
    ...
    SDL_Surface *deck[13],*objet[8],*gobutton,*connectbutton, *winner, *loser, *fail; //Ajout des surfaces vanqueur, perdant et échec
  	...
  	connectbutton = IMG_Load("connectbutton.png");
        winner = IMG_Load("vanqueur.png"); //Ajout de l'immage "winner"
        loser = IMG_Load("gameover.png"); // Ajout de l'image "gameover"
        fail = IMG_Load("fail.png"); // Ajout de l'image "fail"
  	strcpy(gNames[0],"-");
  	strcpy(gNames[1],"-");
  	strcpy(gNames[2],"-");
  	strcpy(gNames[3],"-");
    ...
    SDL_Texture *texture_deck[13],*texture_gobutton,*texture_connectbutton,*texture_objet[8],*texture_winner, *texture_loser, *texture_fail;
    ...
    texture_winner = SDL_CreateTextureFromSurface(renderer, winner); // Ajout de la texture de "winner" 
    texture_loser = SDL_CreateTextureFromSurface(renderer, loser); // Ajout de la lexture de "loser"
    texture_fail = SDL_CreateTextureFromSurface(renderer, fail); // Ajout de la texture "fail "
    TTF_Font* Sans = TTF_OpenFont("sans.ttf", 15);
    printf("Sans=%p\n",Sans);

   /* Creation du thread serveur tcp. */
   printf ("Creation du thread serveur tcp !\n");
   synchro=0; // On debloque le thread réseau 
      ...
      if (SDL_PollEvent(&event))
      {
        switch (event.type)
        {
            ...
            case  SDL_MOUSEBUTTONDOWN:
                ...
                //printf("mx=%d my=%d\n",mx,my);
                if ((mx<200) && (my<50) && (connectEnabled==1)) // Bouton de connexion a été appuyé 
                { 
                  // On crée le message d'envoi
                  sprintf(sendBuffer,"C %s %d %s",gClientIpAddress,gClientPort,gName);
                  sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer); //Envoie des informations du client au server 
                  connectEnabled=0;
                }

                else if ((mx>=0) && (mx<200) && (my>=90) && (my<330)) // Le joueur a été selectionné 
                {
                    joueurSel=(my-90)/60;
                    guiltSel=-1;
                }
                else if ((mx>=200) && (mx<680) && (my>=0) && (my<90)) // L'objet a été selectionné 
                {
                    objetSel=(mx-200)/60;
                    guiltSel=-1;
                }
                else if ((mx>=100) && (mx<250) && (my>=350) && (my<740)) // Le coupable a été selectionné 
                {
                    joueurSel=-1;
                    objetSel=-1;
                    guiltSel=(my-350)/30;
                ...
                else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))  // Bouton "go" a été appuyé 
                {
                    printf("go! joueur=%d objet=%d guilt=%d\n",joueurSel, objetSel, guiltSel);
                    if (guiltSel!=-1)
                    {
                      sprintf(sendBuffer,"G %d %d",gId, guiltSel); // Création d'un message à envoyer  : Accusation d'un coupable 
                      sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer); // Envoie de message au serveur  : Indice du coupable  
                    }
                    else if ((objetSel!=-1) && (joueurSel==-1))
                    {
                      sprintf(sendBuffer,"O %d %d",gId, objetSel); // Création d'un message à envoyer : Demande d'objet 
                      sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer); // Envoie de message au serveur : Indice de l'objet selectionné 
                    }
                    else if ((objetSel!=-1) && (joueurSel!=-1))
                    {
                      sprintf(sendBuffer,"S %d %d %d",gId, joueurSel, objetSel); // Création d'un message à envoyer : Demande d'objet à un joueur spécifique
                      sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer); // Envoie de message au serveur : Indice de l'objet et l'indice du joueur selectionnés
                    }
                }
                else
                {
            ...
            case  SDL_MOUSEMOTION:
                SDL_GetMouseState( &mx, &my ); // Récupérer les coordonnées de la souris 
                break;
        }
      }

        // Reception d'un message du serveur sur le thread créé 
        if (synchro==1)
        {
          int joueurCourant;  // Indice du joueur qui communique avec le serveur 
          int i, j, valeur; // Indices dans d'une valeur dans le table carte 
          char ans;
          printf("consomme |%s|\n",gbuffer);
          switch (gbuffer[0])
          {
              ...
              case 'I': 
                  sscanf(gbuffer + 1, "%d", &gId); // Récuperer le message envoyé par le server  dans le buffer de réception
                  break;
	      ...
	      // Cela permet d'affecter goEnabled pour autoriser l'affichage du bouton go
              case 'M': 
                  ...
                  if (gId == joueurCourant)
                    goEnabled = 1; // Activation du bouton Go  
                  else
                    goEnabled = 0;
                  break;
              // le joueur reçoit une valeur de table carte
              case 'V':   
                  sscanf(gbuffer + 1, "%d %d %d", &i, &j, &valeur); // Récupérer la valeur envoyé par le serveur 
                  tableCartes[i][j] = valeur;
                  break;
              // Un des joueurs a gagné 
              case 'W': 
              sscanf(gbuffer + 1, "%d", &joueur_win);
              printf(" numéro du joueur vainquer : %d\n",joueur_win);
              if(gId == joueur_win)
                printf("Vous avez gagné ! \n");
              else
                printf("Joueur gagnat  :  \n",gNames[joueur_win]);
              break;
              // le joueur a échoué l'accusation 
              case 'E': 
              joueur_echec = 1;
              printf("Tentative échouée !\n");
            }
            synchro=0; // Fin de traitement des messages 
            }
